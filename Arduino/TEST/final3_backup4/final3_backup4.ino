...
void checkAndSendUID() {
    ...
    if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
        ...
        String name = "User";
        if (u.startsWith("CCF0C301")) {
            name = "Beomjin Kim";
        }
        else if (u.startsWith("C44C0B05")) {
            name = "tungtung";
        }
        
        Serial.println("[사용자 이름] " + name);
...
void processServerInput() {
    ...
    if (incoming.startsWith("GM")) {
        if (incoming.length() >= 4) {
            String medName = incoming.substring(3);
            medName.trim();
    ...
    } else if (incoming.startsWith("ER")) {
        if (incoming.length() >= 5) {
            String errCode = incoming.substring(3, 5);
            if (errCode == "DB") errText = "[ERROR] DB: DB 오류";
            else if (errCode == "FT") errText = "[ERROR] FT: 데이터 형식 오류";
            else if (errCode == "AU") errText = "[ERROR] AU: 사용자 인증 오류";
            else if (errCode == "CM") errText = "[ERROR] CM: 명령어 오류";
            else if (errCode == "SV") errText = "[ERROR] SV: 서버 내부 오류";
            else errText = "[ERROR] 알 수 없는 오류 코드";
