...
void handleSerialCommand(String cmd) {
    ...
    if (cmd.startsWith("GM")) {
        String medName = cmd.substring(2);
        Serial.println("[약 복용 알림 수신] " + medName);
    ...
    if (cmd.startsWith("ER")) {
        if (cmd.length() >= 6) {
            String status = cmd.substring(2, 3);
            String code = cmd.substring(3, 5);
            String msg = "";

            if (code == "DB") msg = "DB Error (DB: 데이터베이스 오류)";
            else if (code == "FT") msg = "Format Error (FT: 데이터 형식 오류)";
            else if (code == "AU") msg = "Auth Error (AU: 인증 오류)";
            else if (code == "CM") msg = "Unknown Cmd (CM: 명령어 오류)";
            else if (code == "SV") msg = "Server Error (SV: 서버 오류)";
            else msg = "Unknown Error";

            Serial.print("[에러 코드 수신] 상태: ");
            Serial.print(status);
            Serial.print(" / 코드: ");
            Serial.print(code);
            Serial.print(" → ");
