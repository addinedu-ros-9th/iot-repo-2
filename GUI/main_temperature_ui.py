...
class MainTemperatureWindow(QMainWindow):
    def __init__(self, user_id="", target_user_id=None):
        super().__init__()
        uic.loadUi("/home/kbj/dev_ws/project/main_temperature.ui", self)
        self.user_id = user_id
        self.target_user_id = target_user_id or user_id
        self.user_role = ""
        self.center_window()

        print(f"[DEBUG] MainTemperatureWindow 초기화 → 로그인 ID: {self.user_id}, 대상 ID: {self.target_user_id}")
        ...
        self.calendarWidget.selectionChanged.connect(self.update_temperature_graphs)
        ...
        self.temperature_plot = pg.PlotWidget(title="24시간 체온")
        self.temperature_plot.setBackground('w')
        self.temperature_plot.setYRange(30, 40)
        self.temperature_plot.showGrid(x=True, y=True)
        tp_layout = self.temperature_widget.layout() or QVBoxLayout(self.temperature_widget)
        tp_layout.addWidget(self.temperature_plot)

        self.temperature_compare_plot = pg.PlotWidget(title="평균 체온 비교")
        self.temperature_compare_plot.setBackground('w')
        self.temperature_compare_plot.setYRange(30, 40)
        self.temperature_compare_plot.showGrid(x=True, y=True)
        cmp_layout = self.daily_temperature_widget.layout() or QVBoxLayout(self.daily_temperature_widget)
        cmp_layout.addWidget(self.temperature_compare_plot)

        self.update_temperature_graphs()
    ...
    def update_temperature_graphs(self):
        uid = self.get_selected_elderly_id()
        date_str = self.calendarWidget.selectedDate().toString("yyyy-MM-dd")
        payload = {"user_id": uid, "date": date_str}

        try:
            res = requests.post("http://34.64.53.123:9999/query/cal-temp", json=payload)
            res.raise_for_status()
            data = res.json()
            ...
            vals = [float(v.get("temperature", 0)) for k, v in items if 0 <= int(k) <= 23]

            self.temperature_plot.clear()
            self.temperature_plot.setBackground('w')
            self.temperature_plot.setYRange(32, 40)
            self.temperature_plot.setXRange(0, 24)
            self.temperature_plot.showGrid(x=True, y=True)

            # x축 눈금: 2시간 단위 (0, 2, 4, ..., 24)
            ticks = [(i, f"{i}시") for i in range(0, 25, 2)]
            self.temperature_plot.getPlotItem().getAxis('bottom').setTicks([ticks])
            self.temperature_plot.getPlotItem().getAxis('bottom').setHeight(30)

            self.temperature_plot.setTitle("24시간 체온")
            self.temperature_plot.plot(hours, vals, pen=pg.mkPen(width=2, color='orange'), symbol='o', symbolSize=7, symbolBrush='blue')

            # 평균 비교 막대그래프
            avg = float(data.get("average", 0))
            self.temperature_compare_plot.clear()
            self.temperature_compare_plot.setBackground('w')
            self.temperature_compare_plot.setYRange(32, 40)
            self.temperature_compare_plot.showGrid(x=True, y=True)
            self.temperature_compare_plot.addItem(pg.BarGraphItem(x=[0], height=[avg], width=0.5, brush='red'))
            self.temperature_compare_plot.addItem(pg.BarGraphItem(x=[1], height=[36.5], width=0.5, brush='green'))
            self.temperature_compare_plot.getPlotItem().getAxis('bottom').setTicks([[(0, "평균"), (1, "정상(36.5°C)")]])
            self.temperature_compare_plot.getPlotItem().getAxis('bottom').setHeight(40)
            self.temperature_compare_plot.setTitle(f"{date_str} 평균: {avg:.2f} ℃")

        except Exception as e:
            print("[ERROR] 체온 데이터 오류:", e)
